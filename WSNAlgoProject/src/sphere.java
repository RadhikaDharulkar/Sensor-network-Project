...
public class sphere extends PApplet implements java.lang.Cloneable{

	static public void main(String[] args) {
		PApplet.main(sphere.class.getName());
	}
	...
	public void setup() {
		...
		textSize(16);
		output = createWriter("C:/Users/rd28d/Desktop//AlgoOut/b10/colorDistribution.txt");
		output2 = createWriter("C:/Users/rd28d/Desktop/AlgoOut/b10/originaldegre.txt");
		output3 = createWriter("C:/Users/rd28d/Desktop/AlgoOut/b10/avgdegree.txt");
		output4 = createWriter("C:/Users/rd28d/Desktop//AlgoOut/b10/priset.txt");
	...
	public void keyPressed() {
		...
		if (key == CODED) {//down  +x right +y
			if (keyCode == UP) {
				//ydir = ydir + 10;
				rotateAngleX= rotateAngleX - PI/30;
			} else if (keyCode == DOWN) {
				rotateAngleX= rotateAngleX + PI/30;
				//ydir = ydir - 10;
			} else if (keyCode == RIGHT) {
				rotateAngleY= rotateAngleY - PI/30;
				//xdir = xdir - 10;
			...
			for (int i = 0; i < rsp.maxPoints; i++) {
				for (int j = 0; j < rsp.maxPoints; j++) {
					if (i != j) {
						...
						if (dist(rsp.newpoints[0][i], rsp.newpoints[1][i],rsp.newpoints[6][i], rsp.newpoints[0][j], rsp.newpoints[1][j],rsp.newpoints[6][j]) <= r) {
			...
			for (int i = 0; i < rsp.maxPoints; i++) {
				...
				for (int j = 0; j < rsp.maxPoints; j++) {
					if (i != j && rsp.newarray[2][i]==0 && rsp.newarray[2][j]==bptcolor) {
						...
						if (dist(rsp.newarray[0][i], rsp.newarray[1][i],rsp.newarray[3][i], rsp.newarray[0][j], rsp.newarray[1][j],rsp.newarray[3][j]) <= r) {
							localcounter++;
							if(localcounter>=3){
				...
				for (int i = 0; i < rsp.maxPoints; i++) {
					...
					if (rsp.newpoints[3][i] == 1) {
						if (rsp.newpoints[2][i] == min) {
							...
							rsp.newarray[3][(int)(removecounter-1)]= rsp.newpoints[6][i];
							if(clique>0){
								rsp.newpoints[5][i]=1;
							}
							break;
	...
	class randomSpherePoints {
		...
		randomSpherePoints(int pointCount, float sphereRadius) {
			...
			newpoints=new float[7][pointCount]; 
			newarray= new float[4][pointCount];
			...
			for (int ni = 0; ni < maxPoints; ni++) {
				...
				//float u= gen.nextFloat();
				float u = (gen.nextFloat()*2)-1;
				//float u = (float) 1.0;
				int t= gen.nextInt();
				newpoints[0][ni] = randomspherePointX(u,t);
				newpoints[1][ni] = randomspherePointY(u,t);
				...
				newpoints[6][ni] = 225*u;
			}

			float tempx, tempy, temp1, temp2, temp3, temp4,temp5;
		...
		void draw() {
			...
			for (int ni = 0; ni < maxPoints; ni++) {
				...
				if(newarray[2][ni]==bptcolor){
					...
					point(newarray[0][ni], newarray[1][ni],newarray[3][ni]);
				}
			...
			if(slca==false){
				for (int ni1 = 0; ni1 < maxPoints; ni1++) {
					//point (points[ni].x, points[ni].y);
					if (newpoints[3][ni1] == 1 || newpoints[5][ni1]==1) {
						point(newpoints[0][ni1], newpoints[1][ni1], newpoints[6][ni1]);
					}
				}
			}
					...
					for (int j = 0; j < rsp.maxPoints; j++) {
						if (i != j && rsp.newpoints[3][j] == 1 && rsp.newpoints[3][i] == 1 || i!=j && rsp.newpoints[5][i]==1 && rsp.newpoints[5][j]==1) {
							...
							if (dist(rsp.newpoints[0][i], rsp.newpoints[1][i],rsp.newpoints[6][i], rsp.newpoints[0][j], rsp.newpoints[1][j],rsp.newpoints[6][j]) <= r) {
								...
								line(rsp.newpoints[0][i], rsp.newpoints[1][i],rsp.newpoints[6][i], rsp.newpoints[0][j], rsp.newpoints[1][j],rsp.newpoints[6][j]);
					...
					for (int j = 0; j < rsp.maxPoints; j++) {
						if (i != j) {
							if(rsp.newarray[2][i] == bptcolor  &&  rsp.newarray[2][j] == p+1 || rsp.newarray[2][j] == bptcolor  &&  rsp.newarray[2][i] == p+1){          
								if (dist(rsp.newarray[0][i], rsp.newarray[1][i],rsp.newarray[3][i], rsp.newarray[0][j], rsp.newarray[1][j],rsp.newarray[3][j]) <= r) {
									line(rsp.newarray[0][i], rsp.newarray[1][i],rsp.newarray[3][i], rsp.newarray[0][j], rsp.newarray[1][j],rsp.newarray[3][j]);
		...
		float randomspherePointX(float u, float t){
			return 225 *(float)Math.sqrt(1- u*u) * cos((float)Math.toRadians(t));
		}
		
		float randomspherePointY(float u, float t){
			return 225 *(float)Math.sqrt(1- u*u) * sin((float)Math.toRadians(t));
		}
		
		float randomspherePointZ(float u){
			return u;
		}
